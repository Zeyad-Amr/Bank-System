...
public class ProcessDaoImpl implements ProcessDao {
    ...
    @Override
    public List<CashProcess> findAll() {
        // TODO Auto-generated method stub
        return null;
    }

    @Override
    public List<CashProcess> findByNationalId(String nationalId) {
        ...
        try (PreparedStatement preparedStatement = con.prepareStatement(query)) {
            ...
            while (result.next()) {
                CashProcess process = new CashProcess(result.getInt("id"), result.getString("name"),
                        ...
                        result.getDouble("balance"), result.getDate("date"), result.getString("description"),
                        result.getString("payTo"));
    ...
    public void save(CashProcess process) {
        ...
        if (process.getId() > 0) {
            // **************** Update Data **************** //
            String query = "UPDATE processes SET name=?, nationalId=?, amount=?,process=?, balance=?, date=?, description=?, payTo=?, WHERE id=?;";
            try (PreparedStatement preparedStatement = con.prepareStatement(query)) {
                ...
                preparedStatement.setDate(6, Utils.getSqlDate(process.getDate()));
                preparedStatement.setString(7, process.getDescription());
                preparedStatement.setString(8, process.getPayTo());
                preparedStatement.setInt(9, process.getId());
            ...
            // **************** Create Data **************** //
            String query = "INSERT INTO processes  (name, nationalId, amount, process, balance, date, description, payTo) VALUES (?,?,?,?,?,?,?,?);";
            try (PreparedStatement preparedStatement = con.prepareStatement(query)) {
                preparedStatement.setString(1, process.getName());
                preparedStatement.setString(2, process.getNationalId());
                preparedStatement.setDouble(3, process.getAmount());
